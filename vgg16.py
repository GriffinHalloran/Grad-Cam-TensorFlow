...
class vgg16:
    def __init__(self, imgs, weights=None, sess=None, label=1):
        ...
        self.heat_map(self.pool5, self.imgs, label)
    ...
    def heat_map(self, last_layer, img_layer, category):
        #print 'probs shape ', self.probs.get_shape()
        target = self.probs[0, category]
        labels = np.array([1 if i == category else 0 for i in range(1000)])
        cost = tf.reduce_sum((self.probs - labels) ** 2)
        train = tf.train.GradientDescentOptimizer(0.0001).minimize(cost)

        conv_layer_grad = tf.gradients(cost,
                             last_layer,
                             grad_ys=None,
                             name='gradients')[0]
        conv_layer_grad_norm = tf.div(conv_layer_grad, tf.sqrt(tf.reduce_mean(tf.square(conv_layer_grad))) + tf.constant(1e-5))

        gb_grad = tf.gradients(cost,
                             img_layer,
                             grad_ys=None,
                             name='gradients')[0]
        # print 'grads', grads.get_shape()
        #print grads
        self.heatmap = [last_layer, conv_layer_grad_norm, gb_grad]
        return
